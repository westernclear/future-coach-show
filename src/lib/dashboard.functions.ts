...
export const getPlayerDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const [profileResult, walletResult, entriesResult, badgesResult, rewardsResult] =
      await Promise.all([
        context.supabase
          .from("profiles")
          .select(
            "display_name, username, favorite_sport, favorite_team, preferred_league, fantasy_skill_level, onboarding_completed_at",
          )
