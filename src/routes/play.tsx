      ...
      <main className="mx-auto grid max-w-7xl gap-10 px-5 py-12 lg:grid-cols-[0.7fr_1.3fr] lg:px-8 lg:py-16">
        ...
        <section aria-label="Available coaches" className="space-y-3">
          {slate.map((coach) => {
            ...
            return <article key={coach.id} className={cn("flex flex-col gap-4 border p-5 sm:flex-row sm:items-center", selected ? "border-primary bg-primary/5" : "border-border")}><span className="grid size-12 place-items-center rounded-full bg-secondary font-display font-black">{coach.initials}</span><div className="flex-1"><h2 className="font-bold">{coach.name}</h2><p className="text-sm text-muted-foreground">{coach.sport} · Projected {coach.projection} pts</p></div><Button variant={selected ? "default" : "outline"} onClick={() => toggle(coach.id)} disabled={!selected && roster.length >= 3}>{selected ? <><Check /> Selected</> : <>Add coach <ChevronRight /></>}</Button></article>;
          })}
